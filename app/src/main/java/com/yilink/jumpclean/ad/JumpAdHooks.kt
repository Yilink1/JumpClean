...
object JumpAdHooks {
    ...
    private fun hookSplashInstantJump(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        log("✔ SplashActivity 穿透 Hook 已就绪")
    }
    ...
    private fun applyAllUIVisibility(activity: Activity) {
        ...
        if (isFeatureEnabled(activity, KEY_HIDE_DISCOVER_BANNER) || isFeatureEnabled(activity, KEY_HIDE_DISCOVER_TOP_AD)) {
            val collapsingId = getCachedResId(activity, "collapsing_toolbar")
            if (collapsingId != 0) {
    ...
    private fun hookPostDateCacheRead(lpparam: XC_LoadPackage.LoadPackageParam) {
        try {
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        try {
                            ...
                            val tvDateId = getCachedResId(context, "tvDate")
            ...
            log("✔ 帖子年份缓存读取 Hook 已安装")
        } catch (e: Exception) {
            logError("✘ 帖子年份缓存读取 Hook 失败", e)
        }
    }

    private fun safeCallStringGetter(obj: Any, methodName: String): String? {
        return try {
            ...
            XposedBridge.hookAllMethods(adapterClass, "onBindViewHolder", object : XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam) {
                    try {
                        ...
                        logError("设置项绑定拦截异常", e)
                    }
                }
            })
            log("✔ 设置页点击拦截（Title 锚点 + 复用重置）已就绪: ${adapterClass.name}")
        } catch (e: Exception) {
            logError("✘ 设置页点击拦截安装失败", e)
        }
    }
    ...
    private fun showFullscreenSettings(activity: Activity) {
        ...
        val items = listOf(
            ...
            SettingItem(KEY_HIDE_POST_AD, "屏蔽推荐流与帖子内嵌广告"),
